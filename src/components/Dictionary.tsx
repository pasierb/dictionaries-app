...
export class Dictionary extends React.Component<Props> {
	validateEntry = ({ from: origFrom, to: origTo }: Entry) => ({ from, to }: Entry): boolean => {
  ...
  handleEditEntry = (key: string) => ({ from, to }: Entry) => {
    ...
    const ordered = Array.from(dictionary.entries).sort((a, b) => a[0].toLowerCase() > b[0].toLowerCase() ? 1 : -1);

		return (
			<React.Fragment>
				{ordered.map(([from, to]) =>
					<DictionaryEntryForm key={from}
						from={from} to={to}
				...
				<DictionaryEntryForm key={dictionary.entries.size}
					from="" to=""
					validate={this.validateEntry({ from: '', to: '' })}
